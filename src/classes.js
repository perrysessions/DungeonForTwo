...
export const CLASSES = {
  ...
  druid: {
    key: 'druid', name: 'Druid', ability: 'thornBurst', abilityName: 'Thorn Burst',
    abilityCost: 14, passiveId: 'd_pass',
    blurb: 'Nature mage. Erupts thorns and regrows health from kills.',
    art: { body: '#3d6b30', accent: '#8bc34a', trim: '#c8e67a', head: 'antlers' },
    stats: {
      maxHp: 105, maxMana: 90, attackDamage: 8, attackCooldown: 0.44,
      projectileSpeed: 360,
    },
    tree: [
      node('d_pass', 'Regrowth', 'PASSIVE — On kill: chance to restore HP (scales per rank)', { passive: true, maxRank: 4 }),
      node('d_thorn', 'Overgrowth', '+20% Thorn Burst damage', { maxRank: 4, mods: { holy: 0.2 } }),
      node('d_slow', 'Deeproots', '+20% Thorn slow duration', { maxRank: 3, mods: { slowPower: 0.2 } }),
      node('d_regen', 'Verdant', '+0.6 HP regen/sec', { maxRank: 3, stats: { hpRegen: 0.6 } }),
      node('d_spd', 'Swiftleaf', '+8 Move speed', { maxRank: 3, stats: { moveSpeed: 8 } }),
      node('d_hp', 'Bark Skin', '+12 Max HP', { maxRank: 4, stats: { maxHp: 12 } }),
      node('d_dmg', 'Wrath of Nature', '+7% Damage', { maxRank: 4, stats: { damageMult: 0.07 } }),
    ],
  },
  rogue: {
